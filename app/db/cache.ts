...
export class Cache implements ICache{
    ...
    private constructor(){
        if(redisUrl){
            this.delegate= RedisCache.getInstanceOfRedis(redisUrl);
        }
        else{
    ...
    static getInstance(): Cache {
      ...
      async set(type: string, args: string[], value: any, expirySeconds: number=parseInt(process.env.CACHE_EXPIRE_S || '100', 10)): Promise<void> {
